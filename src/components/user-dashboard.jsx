...
export function UserDashBoard(){
    ...
    function handleModalClose() {
        setEditTasks([{ Appointment_Id: 0, Title: '', Description: '', Date: '' }]);
        editFormik.resetForm(); // Reset form values
    }
    const editFormik = useFormik({
        initialValues: {
        ...
        enableReinitialize: true,
        onSubmit: (task) => {
            axios.put(`http://127.0.0.1:7000/edit-task/${task.Appointment_Id}`, task)
                .then(() => {
                    alert('Task Updated Successfully!');
                    setAppointments(prev =>
                        prev.map(apt => (apt.Appointment_Id === task.Appointment_Id ? task : apt))
                    );
                    editFormik.resetForm(); // Clear the form after save
                    handleModalClose(); // Close modal and reset EditTasks
                })
                .catch(err => {
